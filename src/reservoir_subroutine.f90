SUBROUTINE reservoir_subroutine(nresx, nd)
 ...
 ! ---------------- turnover loop driven only by T_epil and T_hyp ----------
        ...
        if ( ( T_epil(nresx) - T_hypo(nresx)) .lt. (2) .and. dayx .gt. 245) then !245 is September 1st
                if( (T_epil(nresx) - T_hypo(nresx)) .lt. (0) ) then
                  ...
                  K_z(nresx) = 0.0001  ! set the diffusion coeff. in m^2/day
  ...
  ! ------------------- calculate change in temperature  ---------------------
      ! ---------------- epilimnion -----------
         ! ------------ calculate total energy ----------
          ...
          temp_change_ep(nresx) = advec_in_epix - advec_epi_hyp + dif_epi_x + energy_x
